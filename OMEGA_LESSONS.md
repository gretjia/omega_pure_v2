...
- **C-077**: **模型输出指标不可包装为数据固有属性——度量数据必须独立于模型**。"2.4% SNR"实际是 Phase 12（一个失败模型，RankIC=-0.021）的 D9-D0 Spread (4.51BP) / Target Std (189.6BP)。它被计算一次后经 15+ 文件传播，获得了"A股物理常数"的权威地位，驱动了废弃 L1、限制 hd、论证 MSE 失效等重大决策。实际上模型变了 SNR 就变了（Phase 13 = 3.7%）。数据侧基线（mean/std/skew/kurtosis）必须独立于任何模型计算（Ω1: 只信实测 + Ω5: 生产者≠验证者——Claude 既计算了 SNR，又用它做了架构论证）
- **C-078**: **三方审计元模式——每一方都用偏爱的理论框架解释同一组数据，只有数据审核员免疫**。架构师用设计愿景解读（把失败方案写成当前方案），外部审计师用容量理论解读（hd=64 欠拟合"恰好解释"7BP），仲裁者用 MDL 教条解读（λ_s=0 是"理论叛逃"，但 6 个 Phase 实证显示 λ_s>0 杀信号）。数据审核员不做因果推断只讲事实，是唯一没被偏见污染的角色。"一个数字比十页理论更有价值"（Ω1: 只信实测 + Ω5: 生产者≠验证者）
- **C-079**: **codex exec 长 prompt 注入（>2KB 内联命令行）导致进程无限卡死**。多次复现：Codex CLI 以 node 子进程启动，命令行参数过长时 API 调用挂起（0 字节输出，进程 sleeping）。**修复：将 prompt 写入临时文件（如 `/tmp/codex_prompt.md`），然后 `codex exec --full-auto "$(cat /tmp/codex_prompt.md)"`**。短 prompt（<1KB）直接内联无问题。这是工具链的物理限制，不是逻辑错误（Ω4: 可执行 > 可记忆 — 把"别写太长"变成"写文件再注入"的标准操作）
- **C-080**: **宏观旁路 A/B 实验 — SRL 遮蔽不是问题，直接拼入 V_D/σ_D 无增量**。Phase 14 Step 2: Arm A (6-dim baseline) Best RankIC=+0.0122, Arm B (8-dim +log1p(V_D)+log1p(σ_D)) Best RankIC=+0.0064。Arm B 每个 epoch 的 RankIC 均低于 Arm A。D9-D0 在 E4 偶发 +9.06BP 但 RankIC 仅 0.005（少数极端预测拉高尾部，整体排序弱）。结论：SRL 反演已充分提取 V_D/σ_D 信息，冗余拼入反而稀释信号。审计疑点 F 终结（Ω1: 只信实测 — 5 epoch A/B 对照，RankIC 差 90%）
