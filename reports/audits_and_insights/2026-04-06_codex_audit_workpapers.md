...
## 十五、Phase 15 实验数据 (2026-04-07 补充)

### 15.1 Step 1: 训练稳定化 (Omega-TIB, grad_accum=16 + EMA)

Job `3385623196454617088`, T4 SPOT, pd-ssd 1000GB staging, 9.5h, ~$8

**变更 vs Phase 13**: grad_accum=16 (有效 batch 4096), EMA(decay=0.999, E10起), embargo_shards=2, steps_per_epoch=4992, staging I/O

| Epoch | Rank IC | D9-D0 (BP) | IC_loss | Std_yhat | Pearson IC | EMA |
|-------|---------|-----------|---------|----------|------------|-----|
| 0 | +0.0099 | +2.69 | -0.012 | 443 | +0.0041 | |
| 1 | +0.0118 | +5.57 | -0.023 | 461 | +0.0068 | |
| 2 | +0.0113 | +4.16 | -0.029 | 624 | +0.0054 | |
| 3 | +0.0065 | +1.20 | -0.013 | 784 | +0.0023 | |
| 4 | +0.0117 | +5.59 | -0.021 | 608 | +0.0072 | |
| 5 | +0.0083 | +5.83 | -0.018 | 495 | +0.0075 | |
| 6 | +0.0096 | +3.93 | -0.015 | 776 | +0.0055 | |
| 7 | +0.0088 | +4.36 | -0.032 | 615 | +0.0064 | |
| 8 | +0.0112 | +4.14 | -0.025 | 719 | +0.0065 | |
| 9 | +0.0121 | +4.63 | -0.030 | 778 | +0.0068 | |
| 10 | +0.0122 | +4.18 | -0.018 | 767 | +0.0066 | EMA start, LR→3e-5 |
| 11 | +0.0125 | +4.62 | -0.026 | 665 | +0.0073 | ✅ |
| **12** | **+0.0134** | +5.42 | -0.022 | 572 | +0.0083 | ✅ **best** |
| 13 | +0.0116 | +7.09 | -0.026 | 420 | +0.0092 | ✅ |
| 14 | +0.0099 | +6.52 | -0.030 | 404 | +0.0084 | ✅ |

**EMA 模型评估**: Rank IC = **+0.0122**, D9-D0 = +4.19 BP, Std_yhat = 765.80 BP

**对照 Phase 13**:

| 指标 | Phase 15 Step 1 | Phase 13 | 含义 |
|------|----------------|----------|------|
| IC_ema | +0.0122 | N/A | Phase 15 新增 |
| IC_best | +0.0134 (E12) | +0.0292 (E9) | Phase 13 best 是选择偏差 |
| IC_mean (E0-14) | +0.0104 | +0.0100 | **均值几乎一致** |
| 震荡振幅 | 0.007 | 0.022 | Phase 15 **3x 更稳定** |
| 全正率 | 15/15 | 15/15 | 同 |

### 15.2 Step 2: MLP Baseline (归因实验)

Job `6510716717570719744`, T4 SPOT, pd-ssd 1000GB staging, 5.3h, ~$7

**模型**: MLPBaseline — LayerNorm(9600) → Linear(9600,512) → GELU → Linear(512,128) → GELU → Linear(128,1)
**参数**: 5,000,705 (vs Omega-TIB 24,581 = **203x**)
**保留**: SRL 物理层 + FRT 变换 (通过 MLPWithFRT adapter)
**去掉**: FWT 拓扑注意力 + 信息瓶颈 (64→32→16) + AttentionPooling
**训练配置**: 与 Step 1 完全相同 (grad_accum=16, EMA, embargo, IC Loss)
...
| 10 | +0.0159 | +10.64 | -0.038 | 52,539 | +0.0154 | EMA start |
...
**EMA 模型评估**: Rank IC = **+0.0159**, D9-D0 = +10.63 BP, Std_yhat = 52,498.93 BP

### 15.3 MLP vs Omega-TIB 正面对比

| 指标 | MLP (5M params) | Omega-TIB (24.6K params) | MLP / Omega | 含义 |
|------|----------------|------------------------|-------------|------|
| **EMA Rank IC** | **+0.0159** | +0.0122 | **130%** | MLP 更强 |
| **EMA D9-D0** | **+10.63 BP** | +4.19 BP | **254%** | MLP 远更强 |
| Best single epoch | +0.0208 (E1) | +0.0134 (E12) | 155% | MLP 更强 |
| Mean (E1-14) | +0.0117 | +0.0104 | 113% | MLP 略强 |
| EMA Pearson IC | +0.0159 | +0.0066 (推算) | 241% | MLP 远更强 |
| 参数量 | **5,000,705** | 24,581 | **203x** | ⚠️ 不公平 |
| Std_yhat (EMA) | 52,499 | 766 | 68x | MLP 方差爆炸 |
| 震荡振幅 | 0.017 | 0.007 | 2.4x | MLP 更不稳定 |

---

## 十六、Omega Kernel 组件级解剖 (审计准备)
...
原始输入 [B, 160, 10, 10]
  │
  ├── Ch 0-4: LOB (Bid_P, Bid_V, Ask_P, Ask_V, Close)
  ├── Ch 7: delta_p (微观价格冲击)
  ├── Ch 8: macro_v_d (20日ADV)
  └── Ch 9: macro_sigma_d (20日ATR)
  │
  ▼ ══════════ SRL 物理层 (0 可学习参数, torch.no_grad) ══════════
  │
  │ Q_hidden = sign(ΔP) × (|ΔP| / (c_i × σ_D))^(1/0.5) × V_D
  │ 输入: Ch7 + Ch8 + Ch9 + c_friction
  │ 输出: q_metaorder [B, 160, 1] → expand → [B, 160, 10, 1]
  │
  ▼ ══════════ FRT 变换 (0 可学习参数, GPU 实时计算) ══════════
  │
  │ Ch0: Bid_P → (Bid-Mid)/Mid × 10000 (BP 偏差)
  │ Ch1: Bid_V → log1p(Bid_V)
  │ Ch2: Ask_P → (Ask-Mid)/Mid × 10000 (BP 偏差)
  │ Ch3: Ask_V → log1p(Ask_V)
  │ Ch4: Close → log(Close/Anchor) × 100 (累计收益%)
  │ q_metaorder → clamp → symlog
  │
  │ 拼接: [LOB_FRT(5) + q_symlog(1)] = 6 维流形 [B, 160, 10, 6]
  │
  ▼ ══════════ 以下是 Omega Kernel 的可学习部分 ══════════
  │
  ├─── input_proj: Linear(6 → 64)              448 参数 (1.8%)
  ├─── post_proj_norm: LayerNorm(64)            128 参数 (0.5%)
  │
  ├─── [VolumeBlockInputMasking: 禁用 mask_prob=0]
  │
  ├─── FWT: FiniteWindowTopologicalAttention
  │    ├── qkv: Linear(64 → 192, no bias)      12,288 参数 (50.0%) ← 半数参数
  │    ├── proj: Linear(64 → 64)               4,160 参数 (16.9%)
  │    ├── RPB 表: [1197, 4]                    4,788 参数 (19.5%)
  │    └── Pre-LN: LayerNorm(64)                 128 参数 (0.5%)
  │    残差: out = x + FWT(LayerNorm(x))
  │    窗口: (32, 10), 5 个不重叠窗口, 零跨窗口通信
  │
  ├─── Bottleneck:
  │    ├── Linear(64 → 32) + GELU              2,080 参数 (8.5%)
  │    └── Linear(32 → 16)                       528 参数 (2.1%)
  │
  ├─── AttentionPooling(dim=16)                   16 参数 (0.1%)
  │    pool = Σ softmax(z @ W_pool / √16) × z
  │
  └─── IntentDecoder: Linear(16 → 1)              17 参数 (0.1%)
       输出: scalar prediction [B, 1]
...
| 功能域 | 组件 | 参数 | 占比 | 功能 |
|--------|------|------|------|------|
| **特征投影** | input_proj + norm | 576 | 2.3% | 6→64 维映射 |
| **拓扑提取** | FWT qkv + proj + RPB + Pre-LN | **21,364** | **86.9%** | 窗口内注意力 |
| **信息压缩** | Bottleneck | 2,608 | 10.6% | 64→32→16 |
| **决策** | AttentionPool + Decoder | 33 | 0.1% | 16→1 |

**86.9% 的参数用于 FWT 拓扑提取**。如果 FWT 无增量贡献（如 MLP 实验暗示），这 86.9% 的参数可能是浪费。

### 16.3 MLP Baseline 的等效信息流

```
原始输入 [B, 160, 10, 10]
  │
  ▼ ══════════ SRL + FRT (同 Omega, 通过冻结的 adapter) ══════════
  │
  6 维流形 [B, 160, 10, 6]
  │
  ▼ ══════════ MLP (全连接, 无拓扑结构) ══════════
  │
  ├─── Flatten: [B, 160×10×6] = [B, 9600]
  ├─── LayerNorm(9600)
  ├─── Linear(9600 → 512) + GELU           4,915,712 参数 (98.3%)
  ├─── Linear(512 → 128) + GELU               65,664 参数 (1.3%)
  └─── Linear(128 → 1)                           129 参数 (0.0%)
  
  总可学习参数: 5,000,705
```

**关键差异**:
- MLP **展平了** 2D 拓扑结构为 1D 向量 (正是 OMEGA 理论禁止的)
- MLP **没有**窗口注意力、没有瓶颈压缩、没有注意力池化
- MLP 第一层 Linear(9600→512) 吃掉了 **98.3%** 的参数
- MLP 总参数是 Omega 的 **203x**

### 16.4 两个模型在相同输入上学到了什么？(行为差异)

| 行为指标 | Omega-TIB | MLP | 含义 |
|---------|-----------|-----|------|
| Std_yhat (EMA) | 766 BP | 52,499 BP | MLP 预测范围极大 (68x) |
| Pearson IC (EMA) | ~0.007 | ~0.016 | MLP 在绝对值上更准 |
| Rank IC (EMA) | 0.0122 | 0.0159 | MLP 排序更准 |
| D9-D0 (EMA) | 4.19 BP | 10.63 BP | MLP 分离力更强 |
| 震荡振幅 | 0.007 | 0.017 | Omega 更稳定 |
| EMA 阶段趋势 | E12 达峰后下降 | E10-14 持续高位 | MLP 在 EMA 阶段更持续 |
...
| 假说 | 含义 | 验证方法 |
|------|------|---------|
| A: 纯粹是参数量 (203x) | Omega 的拓扑偏置有价值，只是被 24.6K 限制了 | 容量匹配 MLP (~25K-100K) |
| B: 展平 > 拓扑 | 保留 2D 结构反而限制了表达力 | 容量匹配 MLP (~25K) |
| C: Omega 的瓶颈 (64→16) 过度压缩 | 信号被压缩层破坏 | Omega hd=128 (瓶颈 128→64→32) |
| D: 混合效应 | A+B+C 的组合 | 需要多个对照实验 |

**容量匹配 MLP 实验设计** (待执行):
- MLP-small: Linear(9600→64) → GELU → Linear(64→16) → GELU → Linear(16→1) ≈ **615K** params (仍比 Omega 大 25x)
- MLP-tiny: Linear(9600→16) → GELU → Linear(16→1) ≈ **154K** params (6x Omega)
- MLP-micro: Linear(960→16) → GELU → Linear(16→1) ≈ **15K** params (spatial avg first, <Omega)

---

## 十七、Omega Kernel 各组件的实验证据汇总 (审计准备)

**以下表格汇总了项目全生命周期 (Phase 3-15) 中，每个 Omega Kernel 组件的实验证据。**

### 17.1 SRL 物理层 (AxiomaticSRLInverter)

| 实验 | 结果 | 结论 | Phase |
|------|------|------|-------|
| SRL vs SRL+raw macro 旁路 | Arm A (SRL only) +0.0122 > Arm B (SRL+raw) +0.0064 | SRL 压缩充分，旁路有害 | 14 Step 2 |
| c 全局常数 vs per-stock | Phase 0.5 改为 per-stock，Phase 13 成功 | per-stock 是正确方向 | 0.5→13 |
| δ=0.5 是否最优 | **从未测试过 δ≠0.5** | 无数据 | — |
| SRL 是否整体有益 vs 无 SRL | **从未测试过** (MLP 保留了 SRL) | 无数据 | — |

### 17.2 FRT 变换 (Financial Relativity Transform)

| 实验 | 结果 | 结论 | Phase |
|------|------|------|-------|
| FRT vs 原始特征 | Phase 12+ 引入 FRT 后信号稳定 | FRT 有正向贡献 | 12→13 |
| FRT 是否整体有益 vs 无 FRT | **从未测试过** (MLP 也保留了 FRT) | 无数据 | — |

### 17.3 FWT 拓扑注意力 (FiniteWindowTopologicalAttention)

| 实验 | 结果 | 结论 | Phase |
|------|------|------|-------|
| 有 FWT vs 无 FWT (MLP) | MLP (无 FWT) IC=0.0159 > Omega (有 FWT) IC=0.0122 | **FWT 无正向增量** (但容量混淆) | 15 Step 2 |
| 窗口 (4,4) vs (32,10) | Phase 3 (4,4) FVU≈1.0; Phase 6 HPO 选出 (32,10) | (32,10) 更优 | 3→6 |
| 单层 vs 多层 | **从未测试过** | 无数据 | — |
| 跨窗口通信 | **从未测试过** | 无数据 | — |
| RPB 有无 | 创世版无 RPB → 17min 后添加 | RPB 必要 (置换不变性) | 0 |
| Pre-LN 残差 | Phase 12 无 → Phase 13 加入 | 必要 (修复 RPB 梯度死亡) | 12→13 |

### 17.4 信息瓶颈 (Epiplexity Bottleneck)

| 实验 | 结果 | 结论 | Phase |
|------|------|------|-------|
| hd=64 vs 128 vs 256 | Phase 6 HPO: 64≈128, **但 Phase 6 已作废** | **无有效数据** | 6 (失效) |
| hd=64 在修复后管线 | **从未对比过** | 无数据 | — |
| 瓶颈 64→32→16 vs 无瓶颈 | MLP 无瓶颈且更强 (但容量混淆) | **瓶颈可能有害** | 15 Step 2 |
| lambda_s (L1) | Phase 3-13: 每次 λ>0 杀信号 | λ=0 正确 (3 条独立数学证明) | 3→13 |

### 17.5 池化层 (AttentionPooling)

| 实验 | 结果 | 结论 | Phase |
|------|------|------|-------|
| AttentionPool vs GlobalMeanPool | Phase 6 T29 GMP: pred_std=790 爆炸; Phase 13 AttentionPool: 受控 | AttentionPool 必要 | 6→13 |
| AttentionPool vs Flatten+MLP | MLP (Flatten) 更强 (但容量混淆) | **不确定** | 15 Step 2 |

### 17.6 IC Loss

| 实验 | 结果 | 结论 | Phase |
|------|------|------|-------|
| IC Loss vs MSE 变种 | 7 Phase 全败 (MSE/Huber/Softmax/Spear); Phase 13 IC Loss 成功 | IC Loss 唯一有效 | 3→13 |
| IC Loss vs Tail-Weighted IC | **从未测试过** (3 份代码级方案已设计) | 无数据 | — |
| IC Loss + grad_accum=16 | Phase 15: 震荡 3x 减少，均值不变 | 大 batch 稳定但不提升 | 15 Step 1 |

---

## 十八、Phase 13 IC=0.029 的选择偏差分析

### 18.1 统计证据

Phase 13 和 Phase 15 Step 1 使用**几乎相同的模型**（仅训练方法不同），但 IC 峰值差异巨大:

| 指标 | Phase 13 | Phase 15 Step 1 | 差距原因 |
|------|---------|----------------|---------|
| Best Rank IC | +0.0292 | +0.0134 | Phase 13 = 15 次抽样的 max |
| Mean Rank IC | +0.0100 | +0.0104 | **均值一致** |
| Std of Rank IC | 0.0067 | 0.0019 | Phase 15 震荡 3x 小 |

Phase 13 E9=0.029 与 Phase 15 E12=0.013 的差距 (0.016) ≈ Phase 13 的 2.4 个标准差。在正态分布下，15 次抽样的最大值的期望约为 mean + 2.1σ = 0.010 + 2.1×0.007 = **0.025**。Phase 13 的 0.029 在统计上完全合理为选择偏差。

### 18.2 Phase 15 Step 1 为何峰值更低？

grad_accum=16 使**每个 epoch 的验证 IC 更接近真实值**（更低方差）。Phase 13 在高方差环境下"碰运气"碰到了 0.029。Phase 15 在低方差环境下看到了真实的 ~0.013。

这不是 Phase 15 更差——**两者的真实信号强度一样**（均值 ~0.010）。Phase 15 只是更准确地测量了它。
...
| 变量 | 当前状态 | 需要的实验 |
|------|---------|----------|
| **FWT 拓扑 vs 展平** | MLP (展平) > Omega (2D)，但 **203x 容量混淆** | 容量匹配 MLP |
| **瓶颈 (64→16) 是否过度压缩** | MLP (无瓶颈) > Omega (有瓶颈)，但 **容量混淆** | Omega hd=128 (瓶颈 128→64→32) |
| **SRL 整体贡献 vs 无 SRL** | MLP 保留了 SRL → 从未测试无 SRL | 去 SRL 的 MLP |
| **FRT 整体贡献 vs 无 FRT** | MLP 保留了 FRT → 从未测试无 FRT | 去 FRT 的 MLP |
| **hd=64 vs 128 在修复后管线** | Phase 6 HPO 已作废 | Omega hd=128 对比 |
| **δ=0.5 在 A 股** | 从未测试 | log-log 回归经验验证 |
| **跨窗口通信** | INS-070 提出但从未执行 | overlapping windows / Swin |
| **Tail-Weighted IC Loss** | 3 份代码方案已设计，从未测试 | 与 IC Loss 对比 |
| **容量匹配 MLP** | **最关键的缺失实验** | ~25K-100K params MLP |

---

*Updated 2026-04-07 — Phase 15 Step 1+2 数据补充完毕*
*Audit focus: 要不要重新设计 Omega Kernel？数据已准备，结论待审计。*
